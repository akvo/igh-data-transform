...
class BackfillEngine:
    ...
    def run(self, dry_run: bool = False):
        ...
        for table_name, table_data in tables_with_temporal.items():
            ...
            if self.stats["tables_processed"] > len(backfilled_tables):
                backfilled_tables.append(table_name)
        print()
    ...
    def _backfill_table(
        ...
        # Check if the table actually has any of the temporal columns.
        # If not (e.g., a test database with minimal columns), copy as-is.
        raw_cursor.execute(f"PRAGMA table_info({table_name})")  # noqa: S608
        actual_columns = {r[1] for r in raw_cursor.fetchall()}
        temporal_cols_present = set()
        for columns in temporal_groups.values():
            for col_info in columns:
                if col_info["column"] in actual_columns:
                    temporal_cols_present.add(col_info["column"])
        if not temporal_cols_present:
            print(f"  No temporal columns found in {table_name} — copying as-is")
            self._copy_table_as_is(output_conn, raw_conn, table_name)
            raw_conn.close()
            return
    ...
    def _copy_table_as_is(
        self,
        output_conn: sqlite3.Connection,
        raw_conn: sqlite3.Connection,
        table_name: str,
    ):
        """Copy a table from raw to output without transformation."""
        raw_cursor = raw_conn.cursor()
        raw_cursor.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name=?",
            (table_name,),
